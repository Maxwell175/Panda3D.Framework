...
Per the wrap rule ([01](01-abstractions.md)): the native control surface stays close, but `Play`/`Loop` hand back an **`IPlayingInterval`** — a thin handle exposing `Pause`/`Resume`, settable `Time`/`PlayRate` (seek and rate), `Finish`, and an awaitable, observable `Completed`, with `Native` as the escape hatch to the raw `CInterval`. The framework adds only what's missing: composition ergonomics, managed participation, awaitability, and DI-scoped management.
...
public abstract class ManagedInterval : IInterval {           // derive for arbitrary stepped logic
    protected ManagedInterval(double duration, bool openEnded = true, string? name = null);  // openEnded default TRUE
    ...
    public virtual  void Interrupt() { }                      // ET_interrupt (auto-pause/finish teardown)
}
...
// ParticleInterval is defined in the Panda3D.Framework.Particles package next to ParticleEffect, and composes here.
public sealed class ParticleInterval : ManagedInterval {      // HOLDS the timeline while a particle effect runs
    public ParticleInterval(ParticleEffect effect,             // the Particles package's effect type
                            NodePath parent, double duration, double softStopT = 0, bool cleanup = false);
    // Initialize: reparent under `parent` + SoftStart. At duration - softStopT: SoftStop (emission off, live
    // particles drain). Complete/Interrupt: SoftStop (+ optionally cleanup/dispose the effect).
}

// ---- Playback handle: hides the native CInterval behind the controls a game needs ----
public interface IPlayingInterval {
    IObservable<Unit> Completed { get; }   // fires once at the final state, then completes; late subscribers still get it
    bool IsPlaying { get; }                // false at the final state; a looped interval stays true
    double Time { get; set; }              // playhead seconds; the setter seeks (poses at that time)
    double PlayRate { get; set; }          // 1 = normal, negative = reverse
    void Pause();
    void Resume();
    void Finish();                         // jump to the end, firing completion
    CInterval Native { get; }              // escape hatch — the raw native interval
}

// ---- Manager: DI-scoped; owns a constructed CIntervalManager + the step task ----
public interface IIntervalManager : IDisposable {
    IPlayingInterval Play(IInterval interval);                 // flatten (composites -> one native CMetaInterval), attach, Start()
    IPlayingInterval Loop(IInterval interval);                 // same, but native Loop() — the handle never completes
    void Interrupt();                                          // pause/finish everything tagged auto-pause/auto-finish
    // No PlayAsync/WhenDone: await completion via the handle -> `await intervals.Play(x).Completed`.
}
...
var play = intervals.Play(cutscene);                    // IPlayingInterval handle
skip.Clicked.Subscribe(_ => play.Finish());             // skip = Finish(): jumps to the end state
await play.Completed;                                   // awaitable observable; resumes in the gameplay slot
```
Skippable, pausable (`play.Pause()`), scrubbable (`play.Time = 2.5` — a cutscene editor is a slider bound to `Time`), rate-adjustable (`play.PlayRate = 0.25`), and reusable (`intervals.Play(cutscene)` again later replays it), all on the `IPlayingInterval` handle (`play.Native` for the raw `CInterval`).
...
- **Done-event lifecycle.** Launch assigns a unique `SetDoneEvent("ival-done-{id}")` before `Start()`; completion queues that event on the manager's queue — which **defaults to the global queue**, so it flows through the [Events](06-events.md) pump with zero config. `IPlayingInterval.Completed` is built **lazily**: only the first await/subscribe makes the handle subscribe to the done-event (an already-final handle, `GetState() == S_final`, completes immediately), re-checking `S_final` after subscribing to close the check-then-subscribe race and disposing the subscription once it fires. A fire-and-forget `Play` that is never observed subscribes to nothing. `Completed` is an `AsyncSubject`, so late subscribers to an already-finished interval still receive the completion.
...
- **`Finish()` vs `Pause()` vs `ClearToInitial()` on teardown.** `Finish` runs the timeline to its end state (doors end open); `Pause` freezes mid-flight; `ClearToInitial` rewinds to the pristine start. `Finish`/`Pause` are on the `IPlayingInterval` handle; `ClearToInitial` is native, reached via `play.Native`. `Interrupt()` applies each interval's own `auto_pause`/`auto_finish` tag — set the tag at authoring time for per-interval transition behavior.
...
**See also.** [07 Scheduling & Time](07-scheduling-and-time.md) (the step task, `FrameSlots.Intervals`); [06 Events](06-events.md) (done-event → pump → `IPlayingInterval.Completed`); [09 Actors & Animation](09-actors-animation.md) (`CLerpAnimEffectInterval` blends animation weights inside these timelines); [12 Audio & Misc](12-audio-misc.md) (sound cues via `Call`/`ManagedInterval`).
