...
**Bindings and polling are always physical.** In-game controls are about ergonomics — *where* a key sits under the hand — so a binding to `KeyboardButton.AsciiKey('w')` matches the physical QWERTY-W position on every layout, and WASD is correct for AZERTY/Dvorak users with zero ceremony. There is no mapped-binding mode and no `physical:` flag; the runtime matches the raw scan stream. (This is the SDL "bind by scancode" consensus.)

**Layout matters only for display.** To show the player which key to press ("press **[X]** to jump"), ask `IButtonLabels.Label(handle)`, which reads the focused window's `ButtonMap` (`GraphicsWindow.get_keyboard_map()` → `get_mapped_button_label`) and returns the user's actual legend — "Z" on AZERTY for the physical-W key, "W" on QWERTY. Binding ≠ display: only display consults the layout.
...
2. **Interactive rebind:** `IInput.CaptureNext()` returns the next physical control the user presses; the rebind UI then mutates the list — `action.Bindings.Clear(); action.Bindings.Add(new ButtonBinding(captured));`. No capture-mode subsystem, no override layer.
3. **Display:** label the current binding via `IButtonLabels.Label(handle)`, so the user sees their physical keyboard's legend, not a US-QWERTY name.

**Public surface.**
```csharp
// ---- Layer 1: raw device polling (the simple path; per-view) ----
public interface IInput {                                  // resolved from a view scope; default view at root
    bool IsDown(int b);                                    // physical (raw); b from KeyboardButton.* / MouseButton.*
    bool Pressed(int b);                                   // edge: down this frame, up last
    bool Released(int b);
    ...
    int? CaptureNext();                                     // next physical control the user presses (for rebind UI); null until one arrives
}
...
// Buttons are Panda ButtonHandles, produced by the engine's own registries and marshaled as int indexes — no framework enum/constants:
//   KeyboardButton.Space() / KeyboardButton.AsciiKey('w'),  MouseButton.One(),  GamepadButton.FaceA() / .Joystick(0)
...
public sealed class CompositeAxisBinding : IAxisBindingSource {   // two buttons -> float [-1,1]; makeup enforced by ctor
    public CompositeAxisBinding(int negative, int positive);
    public int Negative { get; set; }  public int Positive { get; set; }
}
public sealed class CompositeVectorBinding : IVectorBindingSource {  // four buttons -> LVector2f; makeup enforced by ctor
    public CompositeVectorBinding(int up, int down, int left, int right);
    public int Up { get; set; }  public int Down { get; set; }  public int Left { get; set; }  public int Right { get; set; }
}
...
public interface IButtonLabels { string Label(int physical); }   // focused window's ButtonMap.get_mapped_button_label
...
public static class InputServiceCollectionExtensions {
    ...
    public static IInputContext AddContext(this IServiceProvider sp, string name, int priority = 0);
}
```
...
jump.Bindings.Add(new ButtonBinding(KeyboardButton.Space()));
jump.Bindings.Add(new ButtonBinding(GamepadButton.FaceA()));        // multiple bindings, OR'd
...
move.Bindings.Add(new CompositeVectorBinding(                       // WASD, makeup enforced by the ctor
    KeyboardButton.AsciiKey('w'), KeyboardButton.AsciiKey('s'),
    KeyboardButton.AsciiKey('a'), KeyboardButton.AsciiKey('d')));
...
var captured = input.CaptureNext();
jump.Bindings.Clear();
jump.Bindings.Add(new ButtonBinding(captured));
```

**`dataLoop` registration (conceptual).**
```csharp
public static IServiceCollection AddInput(this IServiceCollection s) =>
    s.AddSingleton<IDevices, Devices>()                    // wraps InputDeviceManager (+ hotplug events)
     .AddScoped<IInput, ViewInput>()                       // per-view: reads that view's MouseWatcher
     .AddHostedTask(name: "dataLoop", sort: FrameSlots.DataLoop, run: (sp, ct) => {
         sp.GetRequiredService<IDataGraph>().Traverse();    // DataGraphTraverser over the data root
         sp.GetRequiredService<IInputRuntime>().Evaluate(); // sample devices → resolve actions → raise events
...
- The mouse/keyboard chain (`MouseAndKeyboard(window,i) → MouseWatcher`) is **per-view scope** — each view has its own `MouseWatcher`, so `IInput` is view-scoped (resolved from `view.Services` via the seeded `ViewContext`, [03](03-rendering.md)) and `MousePosition`/`IsOverUi` answer "in *this* view." For the common single-window app the root provider falls back to the main view, so prototypes don't think about scopes.
...
- **Buttons and axes are two systems, mirroring Panda.** Panda registers gamepad digital buttons (`face_a`, `lshoulder`, `dpad_up`, `start`, …) in the *same* `ButtonRegistry` as keyboard/mouse, so a single `ButtonHandle` registry spans all three input kinds; in the generated C# API these handles are used as integer indexes at the framework boundary. Analog inputs are a *separate* `InputDeviceAxis` system (`LeftX/Y`, `LeftTrigger`, `RightX/Y`, `RightTrigger`, plus `Throttle`/`Rudder`/`Wheel`/`Brake` for sim controllers), read as `AxisState` values. Triggers exist as both a digital button (`GamepadButton.LTrigger()`) and an analog axis (`InputDeviceAxis.LeftTrigger`), exactly as Panda exposes them. The platform backends (XInput, evdev with its per-device quirks table, IOKit) already normalize real hardware to these canonical names — Panda is doing the SDL-gamecontroller-style mapping for us, so we don't re-map per controller.
...
- **Buttons come from Panda's own registries — no framework enum.** `KeyboardButton.Space()`/`AsciiKey('w')`, `MouseButton.One()`, `GamepadButton.FaceA()`/`Joystick(0)` are all `PUBLISHED` static factories returning the generated handle index, so the engine's classes *are* the discoverable constant set; a parallel enum would only re-expose them and fall out of sync (letters have no per-key handle — `ascii_key` is open-ended, as are `joystick(n)` and `Char`). Bindings/polling are physical (raw); display labels come from `IButtonLabels` over the window's `ButtonMap`.
